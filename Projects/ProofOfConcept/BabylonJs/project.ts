...
class Playground { public static CreateScene(engine: BABYLON.Engine, canvas: HTMLCanvasElement): BABYLON.Scene {
    ...
    const csoundLoadTimer = setInterval(() => {
        if (!!document.csoundLoadStarted && !document.csoundLoadFailed && !document.csound) {
            ...
            csound.compileCsdText(csd).then((csoundErrorCode) => {
...
instr 1
    ...
    event_i("i", 2, 0, -1) // clear signals
    event_i("i", 6, 0, -1) // mix instruments into auxes
    event_i("i", 10, 0, -1) // mix signals
 ...
 *********************************************************************************************************************/
 ...
 #define AF_3D_AUDIO__SPEED_OF_SOUND #343# // Meters per second at 20 B0C
