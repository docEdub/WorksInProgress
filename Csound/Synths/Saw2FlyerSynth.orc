...
instr INSTRUMENT_ID
    ...
    elseif (iEventType == EVENT_NOTE_OFF) then
        // If a note was generated to handle a flyer directin of both, Send it's note off event to match this note's
        // note off event.
    ...
    elseif (iEventType == EVENT_NOTE_ON) then
        ...
        // A negative velocity signals flyer direction 2 (counterclockwise), which means this note was generated by a
        // note seeing flyer direction 3 (both). That note event handles the clockwise flyer direction and generated
        // this note event with the same pfields, except the velocity's sign is reversed.
        iFlyerDirection init FLYER_DIRECTION_UNKNOWN
        iIsFlyerDirectionClockwise = true
        if (iVelocity < 0) then
            iVelocity = abs(iVelocity)
            iIsGenerated = true
            iIsFlyerDirectionClockwise = false
        else
            iIsGenerated = false
            iFlyerDirection = CC_VALUE_i(flyerDirectionComboBoxIndex)
            if (iFlyerDirection == FLYER_DIRECTION_BOTH) then
                ; prints("Generating note %.4f\n", p1 + 0.0001)
                event_i("i", p1 + 0.0001, 0, -1, p4, p5, -p6)
            elseif (iFlyerDirection == FLYER_DIRECTION_COUNTERCLOCKWISE) then
                iIsFlyerDirectionClockwise = false
            endif
            kIsGeneratedNoteReleased init false
        endif
        ; prints("iFlyerDirection = %d  p1 = %.4f\n", iFlyerDirection, p1)

        if (iFlyerDirection == FLYER_DIRECTION_BOTH) then
            kIsReleased = release()
            if (kIsReleased == true && kIsGeneratedNoteReleased == false) then
                ; printsk("Releasing generated note %.4f from note %.3f\n", -p1 - 0.0001, p1)
                event("i", -p1 - 0.0001, 0, 0)
                kIsGeneratedNoteReleased = true
            endif
        endif
        ...
        if (CC_VALUE_k(positionEnabled) == true) then
            ...
            if (iIsFlyerDirectionClockwise == false) then
                log_i_trace("iIsFlyerDirectionClockwise == false")
                kZ = -kZ
            endif
